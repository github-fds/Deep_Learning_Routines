#!/usr/bin/env python
"""
This file contains Python interface of deconvolution_2d.
"""
...
__date__       = "2020.09.30"
__description__= "Python interface of deconvolution_2d"
...
def GetOutputSizeOfDeconvolution2d( in_size
    ...
    if rigor or dlr_common.rigor:
       if (in_size<1):
           err+=1
           if verbose: dlr_common.DpuError(f"in_size should be positive: {in_size}", flush=True)
       if (kernel_size<1):
           err+=1
           if verbose: dlr_common.DpuError(f"kernel_size should be positive: {kernel_size}", flush=True)
       if ((kernel_size%2)!=1):
           err+=1
           if verbose: dlr_common.DpuError(f"kernel_size should be odd: {kernel_size}", flush=True)
       if (stride<1):
           err+=1
           if verbose: dlr_common.DpuError(f"stride should be larger than 0: {stride}", flush=True)
       if (padding<0):
           err+=1
           if verbose: dlr_common.DpuError(f"padding should be positive: {padding}", flush=True)

    out_size = int((in_size-1)*stride-2*padding+dilation*(kernel_size-1)+1)
    if err>0: return False, out_size
    else:     return True,  out_size
    # refer to deconv: out_size = stride*(in_size-1)+kernel-2*padding
    # refer to conf  : out_size = (in_size+2*padding-kernel)/stride +1

#===============================================================================
def Deconvolution2d( out_data    # out_channel x out_size x out_size
    ...
    if rigor or dlr_common.rigor:
       ...
       if (out_data.ndim!=3):
           error += 1
           if verbose: dlr_common.DpuError("out_data is not 3 dim", flush=True)
       if (in_data.ndim!=3):
           error += 1
       ...
       if (stride<1):
           error += 1
           if verbose: dlr_common.DpuError(f"stride should be >=1: {stride}", flush=True)
       if (padding<0):
           error += 1
           if verbose: dlr_common.DpuError(f"padding should be >=0: {padding}", flush=True)
       ...
       t_stride      = stride
       t_padding     = padding
       status, t_out_size_expect = GetOutputSizeOfDeconvolution2d( in_size=t_in_size
       ...
       if (t_out_size!=t_out_size_expect):
           error += 1
           dlr_common.DpuError(f"out_size mis-match: {t_out_size, t_out_size_expect}", flush=True)
       if ((t_kernel_size%2)!=1):
           error += 1
           dlr_common.DpuError(f"kernel_size should be odd: {t_kernel_size}", flush=True)
       if verbose:
    ...
    if out_data.dtype.type == np.int32:
        ...
        return False
    _Deconv2d=dlr_common.WrapFunction(dlr_common._dlr
                            ,_fname
                            , None          # return type
...
if __name__=='__main__':
    def TestDeconvolution2d(_dtype):
        ...
        stride = 1
        padding = 0
        status, out_size = GetOutputSizeOfDeconvolution2d( in_size
                                                       , kernel_size
                                                       , stride
                                                       , padding )
        ...
        status = Deconvolution2d( out_data    # out_channel x out_size x out_size
                              , in_data     # in_channel x in_size x in_size
