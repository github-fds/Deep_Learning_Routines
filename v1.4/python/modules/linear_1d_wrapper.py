...
def Linear1d( out_data    # out_size
    ...
    if rigor:
       ...
       if (t_out_size!=t_weight_size_row):
           error += 1
           dlr_common.DlrError(f"row mis-match {t_out_size}:{t_weight_size_row}", flush=True)
       if (t_in_size!=t_weight_size_col):
           error += 1
           dlr_common.DlrError(f"col mis-match {t_in_size}:{t_weight_size_col}", flush=True)
       if verbose:
...
#===============================================================================
if __name__=='__main__':
    dlr_common.DlrPrint("Testing Linar1d", flush=True)
