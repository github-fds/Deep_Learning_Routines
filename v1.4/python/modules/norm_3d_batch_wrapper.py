...
def Norm3dBatch( out_data     # in_channel x <N dimemsion>
    ...
    if rigor:
       ...
       if (running_mean.ndim!=1):
           error += 1
           if verbose: dlr_common.DlrError("running_mean dimension mis-match", flush=True)
       if (running_mean.size!=in_data.shape[0]):
           error += 1
           if verbose: dlr_common.DlrError("running_mean size mis-match", flush=True)
...
#===============================================================================
if __name__=='__main__':
    dlr_common.DlrPrint("Testing Norm3dBatch", flush=True)
