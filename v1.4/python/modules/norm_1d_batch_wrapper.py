#!/usr/bin/env python
"""
This file contains Python interface of norm_1d_batch.
"""
...
__date__       = "2020.09.30"
__description__= "Python interface of norm_1d_batch"
...
#===============================================================================
def Norm1dBatch( out_data     # in_channel x in_size (contiguous)
    ...
    if rigor:
       ...
       if (out_data.ndim!=in_data.ndim):
           error += 1
           if verbose: dlr_common.DlrError("out_data in_data dimension mis-match", flush=True)
       if (running_mean.ndim!=1):
           error += 1
           if verbose: dlr_common.DlrError(f"running_mean dimension mis-match {running_mean.ndim}", flush=True)
       if (running_mean.size!=in_data.shape[0]): # num of channels
           error += 1
           if verbose: dlr_common.DlrError(f"running_mean size mis-match {running_mean.size} {in_data.shape}", flush=True)
       if (running_var.ndim!=1):
           error += 1
           if verbose: dlr_common.DlrError("running_var dimension mis-match", flush=True)
       if (running_var.size!=in_data.shape[0]): # num of channels
           error += 1
           if verbose: dlr_common.DlrError("running_var size mis-match", flush=True)
       if (scale is not None) and (scale.ndim!=1):
           error += 1
           if verbose: dlr_common.DlrError(f"scale should be 1 dim: {scale.ndim}", flush=True)
    ...
    if out_data.dtype.type == np.int32:
        ...
        return False
    _Norm1dBatch=dlr_common.WrapFunction(dlr_common._dlr
                                        ,_fname
                                        , None          # return type
...
#===============================================================================
if __name__=='__main__':
    dlr_common.DlrPrint("Testing Norm1dBatch", flush=True)
